...
namespace HospitalManagement.Areas.Admin.Controllers
{
    public class DoctorScheduleController : BaseController
    {
        ...
        // Lịch khám đang chờ
        public ActionResult Pending(DateTime? fromDate, DateTime? toDate)
        {
            ViewBag.Feature = "Danh sách";
            ViewBag.Element = KeyElement;
            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                var doctorSchedules = workScope.DoctorSchedules.GetAll()
                    .Where(x => x.Status == BookingStatusKey.Pending)
                    .OrderByDescending(x => x.ScheduleBook)
                    .ThenByDescending(x => x.Doctor.Name)
                    .AsEnumerable();
                ...
                if (toDate.HasValue)
                {
                    ...
                    doctorSchedules = doctorSchedules
                        .Where(x => x.ScheduleBook <= to);

                    ViewData["foDate"] = to.ToString("yyyy/MM/dd");
                }
